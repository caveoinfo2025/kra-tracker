-- ============================================================================
-- UAT READ-ONLY PRE-CHECK — Decimal / INR Migration Readiness (Step 3Z)
-- ============================================================================
--
...
--
-- THIS FILE TARGETS UAT ONLY. Do not run it against production. UAT is
-- documented (docs/CHANGELOG.md / docs/NEXT_SESSION.md, Session 9,
-- 2026-06-19, not independently re-verified by this step) as database
-- u686730471_Caveo_UAT, connecting user u686730471_caveouat.
--
-- WHO RUNS THIS: a human/admin with confirmed UAT database access, directly
-- on the UAT server or via a confirmed UAT connection — never from this
-- local dev environment, and never by pasting a UAT DATABASE_URL into a
-- chat session.
--
...
-- then transcribe the relevant numbers into uat-precheck-result-template.md
-- (in this same folder) before sharing back — do not paste raw terminal
-- output containing a hostname, username, or any value you're not sure is
-- safe to share.
--
-- Physical table names below were taken directly from prisma/schema.prisma's
-- @@map directives (confirmed, not guessed) as of Step 3Z (2026-06-23):
...
--
-- NOTE: per Session 9 (2026-06-19), UAT has 97 tables vs. production's 33 —
-- UAT carries additional modules not yet on production. The table list below
-- is the same Release 1/2 in-scope field list used for dev/production; it
-- does not attempt to cover UAT's extra tables.
-- ============================================================================
...
-- Per Session 9, UAT was bootstrapped through migration #19
-- (20260618100000_crm_lead_customer_ref). Confirm whether the 3 newer
...
-- Per Session 9's prod->UAT copy log, expect materially larger counts than
-- dev (e.g. Collection ~141, CrmLead ~280, CrmOpportunity ~49, SalesFunnel
-- ~100, Payment ~26, KRA ~34) — re-confirm current counts, do not assume.
...
-- read the actual numbers below. UAT's data is a prod-mirrored copy, so it
-- may carry different real-world scale than dev's thin smoke-test rows.
...
--     COUNT, as found live in dev — or something else entirely in UAT)
...
--     (this is exactly the dev-found item #16 pattern — confirm whether UAT
--     has the same, a different, or no such mismatch)
SELECT
  kti.id            AS template_item_id,
  kti.targetType,
  km.metricType,

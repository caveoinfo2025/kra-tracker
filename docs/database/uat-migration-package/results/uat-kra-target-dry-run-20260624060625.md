# UAT KRA.target Transform — Dry Run Output

Ran with no CONFIRM_UAT_KRA_TARGET_TRANSFORM set (default dry-run mode). No data written.

```
Target DB name: u686730471_Caveo_UAT
Target DB host (masked): srv2201.***
Mode: DRY RUN (read-only, no confirmation set)
...
DRY RUN complete. 8 row(s) would be updated. No data was written. Set CONFIRM_UAT_KRA_TARGET_TRANSFORM=YES to execute for real.
```

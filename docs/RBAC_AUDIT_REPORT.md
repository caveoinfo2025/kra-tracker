...
|---|---|---|---|---|---|---|
| `/api/admin/crm/{assignment,automation,pipelines,pipelines/[id],sla,territories,territories/[id]}` | GET/POST/PATCH | `getSession()` | Inline `if (!session?.user?.isManager) 403` | **roles.ts-style, despite importing `requirePermission`** | Wrong Permission System | Replace the inline `isManager` check with `requirePermission(session,"Settings","Workflow"` or a dedicated CRM-admin resource — the import is already there, unused. |
| `/api/admin/finance/{advance,collection,conveyance,credit,expenses,policies,voucher}` | GET/POST/PATCH | `getSession()` | Inline `if (!session?.user?.isManager) 403` | **roles.ts-style, despite importing `requirePermission`** | Wrong Permission System | Same — wire to `requirePermission(session,"Settings","Finance",...)`, which already exists in the catalogue. |

This is a distinct and important finding: **7 CRM-admin + 7 Finance-admin route files all `import { requirePermission } from "@/lib/access-control"` and then never call it**, falling back to a bare `isManager` check instead. This means a future `Role` granting "Settings → Finance → EDIT" to a non-manager (e.g. a senior Accounts lead) would have **no effect** on these 7 finance-admin endpoints — they are manager-only no matter what the new permission system says. This is the clearest concrete evidence that `access-control` is not yet the actual source of truth even where it has been partially wired in.

### 3.3 Admin — Identity / Roles / Policies (uses `canAccessSettings` from roles.ts, not access-control, despite governing access-control's own data)

| API Route | Methods | Auth Check | Permission Check | System Used | Risk | Recommendation |
|---|---|---|---|---|---|---|
| `/api/admin/identity/{permissions,permissions/[roleId],policies,policies/[roleId],roles,roles/[id]}` | GET/POST/PATCH | `getSession()` | `if (!canAccessSettings(session.user)) 403` | **roles.ts** (imports `requirePermission` but uses `canAccessSettings` instead) | Wrong Permission System | This is the most ironic instance: the API that lets an admin edit `Role`/`Permission`/`UserRole`/`DataAccessPolicy` rows (the access-control system's own tables) is itself gated by the *legacy* `roles.ts` predicate, not by `access-control`. Should be `requirePermission(session,"Settings","Identity","EDIT")`, which exists in the catalogue. |
| `/api/admin/identity/users/[id]` | PATCH | `getSession()` | `if (!canAccessSettings(session.user)) 403` | roles.ts | Wrong Permission System | Same. |
| `/api/admin/policies/*` (route, [id], [id]/versions, audit, categories) | GET/PATCH | `getSession()` | `if (!canAccessSettings(session.user)) 403` | roles.ts | Wrong Permission System | Same — `requirePermission(session,"Settings","Policy",...)` exists in catalogue, unused. |
| `/api/admin/policies/evaluate` | POST | `getSession()` | **None found** | none | Needs Review | Confirm this is a read-only dry-run evaluator with no side effects; if it can mutate or leak unpublished policy logic, add a check. |
...
| `/api/master-values` | GET | **None — no `getSession()` call at all** | **None** | none | **Missing Auth** | Fully public endpoint (no auth, no session). Likely intended as shared dropdown-option lookup data, but this is undocumented and unverified — flagged as Needs Review/Missing Auth pending a decision on whether it should require at least a session. |
...
7. **`/api/master-values`** — decide and apply whether a session check is required.

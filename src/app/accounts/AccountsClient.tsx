    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-xs text-gray-400 mb-4">
          {formatINRAsLakhs(row.invoiceValueLakhs)} invoiced · {formatINRAsLakhs(row.amountReceivedLakhs)} received ·
          <span className={balance > 0 ? "text-red-600 font-semibold" : "text-green-600 font-semibold"}> {formatINRAsLakhs(balance)} balance</span>
        </p>
            ...
            {custAdvances.map((a) => (
              <div key={a.id} className="flex items-center justify-between text-sm py-1">
                <span className="text-gray-700">
                  {formatINRAsLakhs(a.amountLakhs)} · {a.mode}{a.referenceNo ? ` · ${a.referenceNo}` : ""}
                </span>
                <button
                  type="button"
            ...
            <div className="space-y-1.5">
              {ledger.map((p) => (
                <div key={p.id} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg px-3 py-2">
                  <div>
                    <span className="font-semibold text-green-700">{formatINRAsLakhs(p.amountLakhs)}</span>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2">
            ...
            <div className="text-lg font-bold text-green-800">
              {formatINRAsLakhs(today?.totalLakhs ?? 0)}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Invoiced",    value: formatINRAsLakhs(totalInvoiced) },
          { label: "Total Received",    value: formatINRAsLakhs(totalReceived),   color: "text-green-700" },
          { label: "Outstanding",       value: formatINRAsLakhs(outstanding),     color: outstanding > 0 ? "text-red-600" : "text-green-600" },
      ...
      {unappliedAdvances.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="text-sm font-semibold text-blue-900 mb-2">
            Unapplied Advances ({unappliedAdvances.length}) · {formatINRAsLakhs(unappliedAdvances.reduce((s, a) => s + a.amountLakhs, 0))} held
          </div>
          <div className="flex flex-wrap gap-2">
            {unappliedAdvances.map((a) => (
              <div key={a.id} className="bg-white border border-blue-200 rounded-lg px-3 py-1.5 text-xs">
                ...
                <span className="text-blue-700 font-semibold ml-2">{formatINRAsLakhs(a.amountLakhs)}</span>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-semibold">
                      {formatINRAsLakhs(r.invoiceValueLakhs, { symbol: false })}
                    </td>
                    <td className="px-4 py-3 text-green-700">
                      {formatINRAsLakhs(r.amountReceivedLakhs, { symbol: false })}
                    </td>
                    <td
                    ...
                    >
                      {formatINRAsLakhs(balance, { symbol: false })}
                    </td>
                    <td
                      className={`px-4 py-3 ${

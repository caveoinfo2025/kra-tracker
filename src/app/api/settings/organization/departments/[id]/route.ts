...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { companyId, name, code, description } = await req.json();
  if (!name?.trim()) return NextResponse.json({ error: "Department name is required." }, { status: 400 });

  try {
    const updated = await prisma.department.update({
      where: { id: Number(id) },
      data: { companyId: Number(companyId), name: name.trim(), code: code?.trim() ?? "", description: description?.trim() ?? "" },
    });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await prisma.department.update({ where: { id: Number(id) }, data: { status } });
    return NextResponse.json(updated);
  } catch {
    return NextResponse.json({ error: "Database not ready." }, { status: 503 });
  }

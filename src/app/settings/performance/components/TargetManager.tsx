...
type Props = { employeeTargets: unknown[]; periods: unknown[]; templates: unknown[] };

export default function TargetManager({ employeeTargets, periods, templates }: Props) {
        ...
        <form onSubmit={handleSubmit}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 500 }}>
              Employee Profile ID *
              <input required value={form.employeeProfileId} onChange={(e) => setForm({ ...form, employeeProfileId: e.target.value })}
                type="number" placeholder="Employee Profile ID"
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                <th style={{ padding: "10px 12px", textAlign: "left", fontWeight: 600, color: "#374151" }}>Employee ID</th>
                <th style={{ padding: "10px 12px", textAlign: "left", fontWeight: 600, color: "#374151" }}>Period</th>
                <th style={{ padding: "10px 12px", textAlign: "left", fontWeight: 600, color: "#374151" }}>Template</th>
            ...
            <tbody>
              {typedTargets.map((t) => (
                <tr key={t.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "10px 12px" }}>#{t.employeeProfileId}</td>
                  <td style={{ padding: "10px 12px" }}>{t.period?.name ?? `Period #${t.periodId}`}</td>
                  <td style={{ padding: "10px 12px" }}>{t.templateId ? `Template #${t.templateId}` : "—"}</td>

...
interface Policy {
  ...
  gstRequired: boolean; panRequired: boolean;
  bankVerificationRequired: boolean; approvalRequired: boolean; status: string;
}

interface Props { canEdit: boolean; currentUserId: number }

export default function VendorGovernance({ canEdit, currentUserId: _uid }: Props) {
  ...
  const [approval, setAppr]   = useState(true);
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/vendor-policy");
      if (res.ok) {
        ...
        if (json.policy) {
          ...
          setBank(json.policy.bankVerificationRequired);
          setAppr(json.policy.approvalRequired);
        }
      }
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/admin/vendor-policy", {
        ...
        body: JSON.stringify({ gstRequired: gst, panRequired: pan, bankVerificationRequired: bank, approvalRequired: approval }),
      });
      if (res.ok) {
  ...
  const fields = [
    { label: "GST Number Required",         desc: "Enforce GST on vendor registration",             value: editing ? gst      : (policy?.gstRequired              ?? true),  setter: setGst  },
    { label: "PAN Required",                desc: "Require PAN for vendor records",                 value: editing ? pan      : (policy?.panRequired              ?? false), setter: setPan  },
    { label: "Bank Verification Required",  desc: "Verify bank account before payment release",     value: editing ? bank     : (policy?.bankVerificationRequired ?? false), setter: setBank },
    { label: "Approval Required",           desc: "Trigger approval workflow for new vendors",      value: editing ? approval : (policy?.approvalRequired         ?? true),  setter: setAppr },
  ];
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600 }}>Vendor Governance Policy</h3>
          <p style={{ margin: "4px 0 0", fontSize: 12, color: "var(--muted-foreground)" }}>
            Global rules applied during vendor onboarding and management.
          </p>
        </div>
        {canEdit && !editing && (
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: 24, maxWidth: 520 }}>
        {fields.map(({ label, desc, value, setter }) => (

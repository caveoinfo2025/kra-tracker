...
export interface ApiExpenseTablePagination {
...
export default function ExpenseTable({
  ...
  search: searchProp, onSearch, apiPagination,
}: {
  rows: Expense[];
  caps: ExpenseCaps;
  ...
  apiPagination?: ApiExpenseTablePagination;
}) {
  const isApiMode = !!apiPagination;
  ...
  const searchVal = isApiMode ? (searchProp ?? "") : localSearch;

  function handleSearchChange(v: string) {
    if (isApiMode) { onSearch?.(v); }
    else { setLocalSearch(v); setLocalPage(1); }
  }
  ...
  const searched = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.expenseNo, r.category, r.subCategory, r.description, r.customer, r.vendor, r.employee, r.voucherNo, r.createdBy]
        .some((v) => (v || "").toLowerCase().includes(q)),
    );
  }, [rows, localSearch, isApiMode]);
  ...
  const localTotalPages = Math.max(1, Math.ceil(sorted.length / PAGE_SIZE));
  const localSafePage   = Math.min(localPage, localTotalPages);
  const pageRows        = isApiMode ? sorted : sorted.slice((localSafePage - 1) * PAGE_SIZE, localSafePage * PAGE_SIZE);

  const displayPage       = isApiMode ? apiPagination.page       : localSafePage;
  const displayTotalPages = isApiMode ? apiPagination.totalPages : localTotalPages;
  const displayTotal      = isApiMode ? apiPagination.total      : sorted.length;

  function goPage(p: number) {
    if (isApiMode) apiPagination.onPageChange(p);
    else setLocalPage(p);
  }
  ...
  function toggleAll() {
    ...
    setSel((s) => { const all = ids.every((i) => s.has(i)); const n = new Set(s); ids.forEach((i) => all ? n.delete(i) : n.add(i)); return n; });
  }
  function runBulk(a: "approve" | "voucher" | "paid") { onBulk(a, [...sel]); setSel(new Set()); }

  const fromRow = isApiMode ? ((apiPagination.page - 1) * apiPagination.pageSize) + 1 : (localSafePage - 1) * PAGE_SIZE + 1;
  const toRow   = isApiMode ? Math.min(apiPagination.page * apiPagination.pageSize, displayTotal) : Math.min(localSafePage * PAGE_SIZE, sorted.length);
    ...
    <div className="card" style={{ overflow: "visible" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderTop: "1px solid var(--border-subtle)", flexWrap: "wrap", gap: 8 }}>
        <div style={{ fontSize: 12, color: "var(--fg-3)" }}>
          {displayTotal === 0 ? "0 results" : `Showing ${fromRow}–${toRow} of ${displayTotal}`}
          {sel.size > 0 && ` · ${sel.size} selected`}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <button className="btn-cav btn-cav-secondary btn-cav-sm" disabled={displayPage <= 1} onClick={() => goPage(displayPage - 1)} style={displayPage <= 1 ? { opacity: 0.4 } : undefined}>Prev</button>
          <span style={{ fontSize: 12, color: "var(--fg-2)", fontWeight: 600 }}>{displayPage} / {displayTotalPages}</span>
          <button className="btn-cav btn-cav-secondary btn-cav-sm" disabled={displayPage >= displayTotalPages} onClick={() => goPage(displayPage + 1)} style={displayPage >= displayTotalPages ? { opacity: 0.4 } : undefined}>Next</button>
        </div>
      </div>
    </div>
  );

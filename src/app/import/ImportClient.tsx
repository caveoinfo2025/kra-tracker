...
const SALES_FIELDS: Record<string, { label: string; required?: boolean; aliases: string[] }> = {
  ...
  dealValueLakhs:    { label: "Deal Value (₹)",                    aliases: ["deal value","booking","booking value","deal amount","value","amount","order value","deal value (lakhs)","booking (lakhs)","deal value (₹)"] },
  billingValueLakhs: { label: "Billing Value (₹)",                 aliases: ["billing value","billing","billing amount","billed amount","billing value (₹)"] },
...
const COLLECTION_FIELDS: Record<string, { label: string; required?: boolean; aliases: string[] }> = {
  ...
  invoiceValueLakhs:     { label: "Invoice Value (₹)",       required: true,  aliases: ["invoice value","amount","total","invoice amount","billed amount","invoice total","bill amount","invoice value (lakhs)","invoice value (₹l)","invoice value (₹)"] },
  amountWithoutGstLakhs: { label: "Total (Without GST) (₹)",                aliases: ["total without gst","without gst","amount without gst","taxable value","taxable amount","net amount","base amount","amount ex-gst","ex-gst","excl gst","excluding gst","amount excl gst","pre-gst amount","amount (without gst)","total (without gst)","without gst (₹l)","without gst (₹)","net value"] },
  dueDate:               { label: "Due Date",                 required: true,  aliases: ["due date","payment due","due by","payment due date","due"] },
  amountReceivedLakhs:   { label: "Amount Received (₹)",                    aliases: ["amount received","received","payment received","collection amount","paid amount","amount paid"] },
